...
from User_Tasks import sync_tasks_c2l
...
async def list_tasks(ctx):
    ...
    if ctx.channel.type == discord.ChannelType.private:
        ...
        if us.user_exists():
            await ctx.channel.send(f"Loading task list for user {ctx.author.name} (Synced from Google Tasks)",delete_after=10)
            try:
                ...
                return
            headings = ['Task Name','Due Date','Tasklist','Status']
            resp = []
            for task_list in tl:
...
async def list_tasklists(ctx):
    ...
    if ctx.channel.type == discord.ChannelType.private:
        ...
        if us.user_exists():
            ...
            resp = []
            for task_list in tl:
                resp.append([task_list['title'], datetime.fromisoformat(task_list['updated']).astimezone().strftime("%B %d, %Y")])
            final_response = table2ascii(header=headings, body=resp)
            final_response = '```\n'+ final_response + '\n```'
            sync_tasks_c2l(str(ctx.author.id))
    ...
    if ctx.channel.type == discord.ChannelType.private:
        ...
        if us.user_exists():
            try:
                ...
                print(task)
                clt.create_task(task_list_id=id_list[tl_no],title=task[0],notes=task[1],due=task[2])
